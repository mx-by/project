...
Still within the CLIMB server, using trimmomatic was used for trimming these reads, specifically the paired end outputs. Its own directory was made 'trimmed_reads', and parameters were set for 4 threads, using both computer CPUs ADD WHAT THE PARAM LINE MEANS
...
The trimmomatic reads were mapped against the progenitor NCBI strain and the differences between the generationally different strains and the progenitor strain using snippy. These were put into their own directory to follow tidy data practises, and all working on the command line was carried out in the CLIMB server and within the necessary directory were analysis was being carried out.
...
snippy --cpus 2 --outdir variant_calling \
  ...
  --R1 trimmed _reads/Y1_P188_02_forward_paired.fq.gz \
